...
/*   Updated: 2025/08/01 13:58:25 by hebatist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_wall_texture_pixel(t_mlx *st_mlx, int screen_column, int y)
{
	...
	tex_offset = tex_y * st_mlx->no_texture->size_line \
		+ st_mlx->st_coord->wall_tex_x * (st_mlx->no_texture->bpp / 8);
	color = *(unsigned int *)(st_mlx->no_texture->img_addr + tex_offset);
