...
/*   Updated: 2025/09/08 14:58:39 by hebatist         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_door_texture_pixel(t_mlx *st_mlx, int screen_column, int y)
{
	...
	if (st_mlx->is_invert)
	{
		tex_y = st_mlx->door_texture->height
			- 1 - (int)st_mlx->current_door->tex_pos;
		if (tex_y < 0)
			tex_y = 0;
		...
		tex_y = (int)st_mlx->current_door->tex_pos;
	st_mlx->current_door->tex_pos += st_mlx->current_door->tex_step;
	tex_offset = tex_y * st_mlx->door_texture->size_line
		+ st_mlx->current_door->tex_x * (st_mlx->door_texture->bpp / 8);
