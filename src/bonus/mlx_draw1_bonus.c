...
/*   Updated: 2025/09/01 20:31:20 by dbatista         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	set_door_texture(t_mlx *st_mlx, int door_line_height)
{
	...
	{
		if (st_mlx->st_file->doors->door_side == 0)
			door_hit_point = st_mlx->st_coord->p_posy
				+ (st_mlx->st_file->doors->perp_door_dist * st_mlx->st_coord->ray_dir_y);
		else
			door_hit_point = st_mlx->st_coord->p_posx
				+ (st_mlx->st_file->doors->perp_door_dist * st_mlx->st_coord->ray_dir_x);
		door_hit_point -= floor(door_hit_point);
		st_mlx->st_file->doors->door_tex_x = (int)(door_hit_point
				* (double)st_mlx->door_texture->width);
		if (st_mlx->st_file->doors->door_side == 0 && st_mlx->st_coord->ray_dir_x > 0)
			st_mlx->st_file->doors->door_tex_x = st_mlx->door_texture->width
				- st_mlx->st_file->doors->door_tex_x - 1;
		if (st_mlx->st_file->doors->door_side == 1 && st_mlx->st_coord->ray_dir_y < 0)
			st_mlx->st_file->doors->door_tex_x = st_mlx->door_texture->width
				- st_mlx->st_file->doors->door_tex_x - 1;
		st_mlx->st_file->doors->door_tex_step = 1.0
			* st_mlx->door_texture->height / door_line_height;
		st_mlx->st_file->doors->door_tex_pos = (st_mlx->st_file->doors->door_ceiling_height
				- (double)st_mlx->screen_height
				/ 2 + (double)door_line_height / 2) * st_mlx->st_file->doors->door_tex_step;
	}
}
...
void	draw_vertical_line(t_mlx *st_mlx, int screen_column)
{
	...
	if (st_mlx->st_file->doors->is_door_col
		&& st_mlx->st_file->doors->perp_door_dist < st_mlx->st_coord->perp_wall_dist
		&& !st_mlx->st_file->doors->is_door_open)
		st_mlx->z_buffer[screen_column] = st_mlx->st_file->doors->perp_door_dist;
	wall_line_height = (int)(st_mlx->screen_height
			/ st_mlx->st_coord->perp_wall_dist);
	door_line_height = (int)(st_mlx->screen_height
			/ st_mlx->st_file->doors->perp_door_dist);
	...
	st_mlx->wall_floor_height = set_heights(wall_line_height,
			st_mlx->screen_height, 1);
	st_mlx->st_file->doors->door_ceiling_height = set_heights(door_line_height,
			st_mlx->screen_height, -1);
	st_mlx->st_file->doors->door_floor_height = set_heights(door_line_height,
			st_mlx->screen_height, 1);
