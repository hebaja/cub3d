...
void	draw_vertical_line(t_mlx *st_mlx, int screen_column)
{
	...
	if (st_mlx->is_door_col
		&& st_mlx->perp_door_dist < st_mlx->st_coord->perp_wall_dist
		&& !st_mlx->is_door_open)
	...
	st_mlx->wall_floor_height = set_heights(wall_line_height,
			st_mlx->screen_height, 1);
	st_mlx->door_ceiling_height = set_heights(door_line_height,
			st_mlx->screen_height, -1);
	st_mlx->door_floor_height = set_heights(door_line_height,
			st_mlx->screen_height, 1);
