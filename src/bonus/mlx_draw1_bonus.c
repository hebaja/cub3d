...
static void	set_door_texture(t_mlx *st_mlx, t_door *door, t_coord *st_coord)
{
	...
	{
		if (door->side == 0)
			door_hit_point = st_coord->p_posy
				+ (door->perp_dist * st_coord->ray_dir_y);
		else
			door_hit_point = st_coord->p_posx
				+ (door->perp_dist * st_coord->ray_dir_x);
		door_hit_point -= floor(door_hit_point);
		door->tex_x = (int)(door_hit_point
				* (double)st_mlx->door_texture->width);
		if (door->side == 0 && st_coord->ray_dir_x > 0)
			door->tex_x = st_mlx->door_texture->width
				- door->tex_x - 1;
		if (door->side == 1 && st_coord->ray_dir_y < 0)
			door->tex_x = st_mlx->door_texture->width
				- door->tex_x - 1;
		door->tex_step = 1.0
			* st_mlx->door_texture->height / door->line_height;
		door->tex_pos = (door->ceiling_height
				- (double)st_mlx->screen_height
				/ 2 + (double)door->line_height / 2) * door->tex_step;
	}
}

static void	set_wall_texture(t_mlx *st_mlx)
{
	...
	st_mlx->st_coord->wall_tex_step = 1.0
		* st_mlx->curr_texture->height / st_mlx->wall_line_height;
	st_mlx->st_coord->wall_tex_pos = (st_mlx->wall_ceiling_height
			- (double)st_mlx->screen_height / 2
			+ (double)st_mlx->wall_line_height / 2)
...
static int	set_heights(int line_height, int screen_height, int changer)
{
	int	height;
...
void	draw_vertical_line(t_mlx *st_mlx, int screen_column)
{
	...
	if (st_mlx->is_door_col)
	{
		st_mlx->current_door->ceiling_height = set_heights(
				st_mlx->current_door->line_height,
				st_mlx->screen_height, -1);
		st_mlx->current_door->floor_height = set_heights(
				st_mlx->current_door->line_height,
				st_mlx->screen_height, 1);
		if (st_mlx->current_door->perp_dist < st_mlx->st_coord->perp_wall_dist
			&& !st_mlx->current_door->is_open)
			st_mlx->z_buffer[screen_column] = st_mlx->current_door->perp_dist;
	}
	st_mlx->wall_line_height = (int)(st_mlx->screen_height
			/ st_mlx->st_coord->perp_wall_dist);
	st_mlx->wall_ceiling_height = set_heights(st_mlx->wall_line_height,
			st_mlx->screen_height, -1);
	st_mlx->wall_floor_height = set_heights(st_mlx->wall_line_height,
			st_mlx->screen_height, 1);
	set_current_texture(st_mlx);
	set_door_texture(st_mlx, st_mlx->current_door, st_mlx->st_coord);
	set_wall_texture(st_mlx);
