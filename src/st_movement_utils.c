...
void	rotate_angle(t_coord *coord, double angle)
{
	...
	coord->dir_vec_x = coord->dir_vec_x * cos(angle) - coord->dir_vec_y *sin(angle);
	coord->dir_vec_y = old_dir_x * sin(angle) + coord->dir_vec_y *cos(angle);
	coord->cam_plane_x = coord->cam_plane_x * cos(angle) - coord->cam_plane_y * sin(angle);
	coord->cam_plane_y = old_plane_x * sin(angle) + coord->cam_plane_y * cos(angle);
}
